...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
		...
		<link href="{{asset('vender/css/customer/footer.css')}}" rel="stylesheet">
		 <link href="{{asset('vender/css/customer/payment.css')}}" rel="stylesheet"> 
	...
	<body> 
		...
		<main class="  ">
			<section class="payment_container px-0 py-5 "  >
				<div> 
					<h1 class="text-center">{{$successMSG->heading}}</h1>
					<p class="text-center">{{$successMSG->message}}</p>
					<div class="d-flex justify-content-center align-items-center py-4 ">
						<a href="{{route('userBack')}}" class="btn btn-dark   mx-auto fw-bold   "     >{{$successMSG->button}}</a>
					</div>
				</div>
			</section>				
		</main>
